...
class ExcelToJsonConverter:
    ...
    def __init__(
        ...
        input_dir: str,
        output_dir: str,
        generate_gdscript: bool = False,
        gdscript_output_dir: Optional[str] = None,
    ):
        """
        初始化转换器
        ...
        self.gdscript_output_dir = (
            Path(gdscript_output_dir) if gdscript_output_dir else None
        )
    ...
    def convert_excel_to_json(
        self, excel_file: Path, sheet_name: str = ""
    ) -> Dict[str, Any]:
        """
        将Excel文件转换为JSON格式
            ...
            if self.generate_gdscript:
                ...
                try:
                    self.gdscript_generator.generate_scripts_from_json(
                        output_file, self.gdscript_output_dir
                    )
...
def main():
    ...
    parser.add_argument("--file", "-f", help="转换单个文件（指定文件路径）")
    parser.add_argument(
        "--generate-gdscript", "-g", action="store_true", help="生成GDScript脚本"
    )
    parser.add_argument(
        "--gdscript-output",
        "-go",
    ...
    if args.file:
        ...
        if (
            file_path.exists()
            and file_path.suffix.lower() in converter.supported_extensions
        ):
